...
class QLambda(object):
    ...
    def train(self):
        ...
        with tqdm(total=self.args['train_epochs'], desc='QLambda Train Progress Bar') as pbar:
            for i in range(self.args['train_epochs']):
                ...
                for (s, a, r, s_n) in samples:
                    self.updateQLambda(s, a, r, s_n, lr=self.args['lr'],
                                           discount_factor=self.args['discount_factor'],
                                           trace_decay=self.args['trace_decay'])

    def evaluate(self, epochs=1000):
        scores = []
        with tqdm(total=epochs, desc='QLambda Evaluate Progress Bar') as pbar:
    ...
    def updateQLambda(self, s, a, r, s_n, lr, discount_factor, trace_decay):
        if self.recent_tuple is not None:
            ...
            # delta = self.recent_tuple[2] + (discount_factor * np.argmax(self.Q_values[(s, a)])) - self.Q_values[(
            #     self.recent_tuple[0], self.recent_tuple[1])]
            # delta = self.recent_tuple[2] + (discount_factor * q_value_n - self.Q_values[(
            #     self.recent_tuple[0], self.recent_tuple[1])]
            delta = self.recent_tuple[2] + (discount_factor * q_value_n - self.Q_values[(self.recent_tuple[0], self.recent_tuple[1])])
