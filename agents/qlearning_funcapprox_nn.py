...
class QLearningFuncApproxNN(object):
    ...
    def write_evals_to_file(self):
        with open('qlearning_funcapprox_nn_evals.pkl', 'wb') as f:
            pickle.dump(self.batch_evals, f)

    def train(self):
        with tqdm(total=self.args['train_epochs'], desc='QLearning Func. Approx. Train Progress Bar') as pbar:
            scores = []
    ...
    def evaluate(self, epochs=1000):
        with tqdm(total=epochs, desc='QLearning Func. Approx. Evaluate Progress Bar') as pbar:
            scores = []
